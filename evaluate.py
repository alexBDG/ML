...
def main():
    ...
    for model in models:
        ...
        print("shape : ",single_model_prediction.shape)
        print("type : ",type(single_model_prediction))
        if extension == '_test.npy':
#        try:
            print("Trying to save the prediction in a submission file...")
            with open(os.getcwd()+'/results/submission_{0}.csv'.format(model_name), 'w') as file:
                file_writer = csv.writer(file, delimiter=',')
                file_writer.writerow(['Id', 'Category'])
                single_model_prediction = np.argmax(single_model_prediction, 1)
                for t in range(len(single_model_prediction)):
                    file_writer.writerow([t, single_model_prediction[t]])
            print("----- done -----")
#            except:
#                print("Error saving the prediction in a submission file !")
...
# Doesn't work because I delete MobileNet !!
